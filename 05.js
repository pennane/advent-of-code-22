...
const applyMove =
  (multiple) =>
  (stacks, [count, from, target]) => {
    const [moving, newFrom] = R.splitAt(count, stacks[from - 1])

    const newTarget = R.pipe(
      R.unless(R.always(multiple), R.reverse),
      R.flip(R.concat)(stacks[target - 1])
    )(moving)
...
const pipeline = R.pipe(
  parseFile,
  R.apply(R.reduce(applyMove(false))),
  R.map(R.head),
  R.join('')
)
...
const pipeline2 = R.pipe(
  parseFile,
  R.apply(R.reduce(applyMove(true))),
  R.map(R.head),
  R.join('')
)
